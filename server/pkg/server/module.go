...
type Config struct {
	...
	TenantIdentifierLocation string
...
func (m *Module) setupConfig(scope string) *Config {
	...
	return &Config{
		...
		IsMultiTenant:            viper.GetBool(util.GetConfigPath(scope, "is_multi_tenant")),
		TenantIdentifier:         viper.GetString(util.GetConfigPath(scope, "tenant_identifier")),
		TenantIdentifierLocation: viper.GetString(util.GetConfigPath(scope, "tenant_identifier_location")),
...
func (m *Module) setUpTenantMiddleware() {
	if !m.config.IsMultiTenant {
		return
	}
	...
	tenantMiddleware := func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			//! Exclude for swagger
			if c.Path() == "/swagger/*" {
				c.Set(constant.ContextTenantIdentifier, "swagger")
				return next(c)
			}
			// Extract the tenant ID from the header
			headerTenantIdentifier := c.Request().Header.Get(constant.HeaderTenantIdentifier)
			if headerTenantIdentifier == "" {
				return c.JSON(http.StatusUnauthorized, response.APIResponse{Message: "Missing X-Tenant header", Data: nil})
			}

			c.Set(constant.ContextTenantIdentifier, headerTenantIdentifier)

			return next(c)
		}
	}

	m.server.Use(tenantMiddleware)
}
...
func (m *Module) logRequest(c echo.Context, v middleware.RequestLoggerValues) error {
	// Extract tenant identifier from the context
	tenantIdentifier, ok := c.Get(constant.ContextTenantIdentifier).(string)
	if !ok || tenantIdentifier == "" {
		m.logger.Error("error extracting tenant identifier from context")
	...
	case "dev":
		m.logger.Info("request",
			...
			zap.String("tenant", tenantIdentifier),
...
func (m *Module) logConfigurations() {
	...
	m.logger.Debug("----- Tenant Configuration -----")
	m.logger.Debug("IsMultiTenant", zap.Bool("IsMultiTenant", m.config.IsMultiTenant))
	if m.config.IsMultiTenant {
		m.logger.Debug("TenantIdentifier", zap.String("TenantIdentifier", m.config.TenantIdentifier))
		m.logger.Debug("TenantIdentifierLocation", zap.String("TenantIdentifierLocation", m.config.TenantIdentifierLocation))
	}
}
